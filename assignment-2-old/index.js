...
var server = http.createServer(function(req, res){
	...
	req.on('end', function(){
		...
		if( paths[0] ) {
			...
			if( typeof handlers.request[method][route][action] !== 'undefined' ) {
				handlers.request[method][route][action]
			}
						...
						fs.open('.data/users/'+buffer.username+'.json', 'r', (err, fd) => {
							if (err) {
								...
								if (err.code === 'ENOENT') { // Error, No Entity/Entry
									...
									/*
									 *
									 The following code is not being used, I was just playing around with some methods to give a user a unique ID
									 Because we do not have a database, and use a file based database it would be nice to possible have a unique ID in the future to connect
									 Other data files to a specific user based on this ID
									 *
									 */
